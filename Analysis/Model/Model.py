...
plt.savefig(f"{OutputPath}/{input_1}_{input_2}_plot.png", 
            bbox_inches='tight', 
            dpi=100, 
            facecolor='white')
...
np_sim_power101 = np.array(sim_power101)
np_sim_power103 = np.array(sim_power103)

import matplotlib.ticker as ticker


plt.figure(figsize=(12,8))
plt.scatter(np.array(time_101), power_101+power_103, label='Combined Device Power', color='purple', s = 1)
# plt.scatter(np.array(time_101), power_101, label='Device 101 Power', color='blue', s = 1)
# plt.scatter(np.array(time_103), power_103, label='Device 103 Power', color='green', s = 1)
plt.scatter(NOAA_time, np_sim_power101+np_sim_power103, label='Combined Sumulated Power', color='green', **kwargs_101)
plt.legend()
plt.title(f'Summed power Output Day {input_2} of 20{input_1}')
ax = plt.gca()
...
plt.ylabel('Power [w]')
plt.ylim(0,800)
plt.savefig(f'PlotsFolder/SummedPowerPlots/{input_1}_{input_2}_summed_plot.png')

print(len(np.array(time_101)))
print(len(np.array(time_103)))